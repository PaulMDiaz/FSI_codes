...
class problem_specific:
	...
	def __init__(self):
		...
		self.T = 0.001#  Set final time for iteration
		self.N = 32		# Number of discretizations (square mesh) (place cell edge on FSI)
		...
		self.bar_2 = Rectangle(Point(0.2,0.19), Point(0.6, 0.21))
		# truncate bar to intersect cylinder on boundary
		self.bar = self.bar_2 - self.cylinder
		...
		self.f_domain = self.channel - self.cylinder - self.bar
...
# Domains that overlap, ie where left end of bar meets FSI part of bar share the corner node.
# Same for where left end ends and the cylinder begins.
		...
		class Left(SubDomain):
		    def inside(self,x,on_boundary):
		        return (x[0]-0.2)**2 + (x[1]-0.2)**2 < 0.05**2 + DOLFIN_EPS and \
				x[0] > 0.2 - DOLFIN_EPS and x[1] < 0.21 + DOLFIN_EPS and \
				x[1] > 0.19 - DOLFIN_EPS and on_boundary

		class Cylinder(SubDomain):
		    def inside(self,x,on_boundary):
				return (x[0]-0.2)**2 + (x[1]-0.2)**2 < 0.05**2 + DOLFIN_EPS and \
				x[0] < 0.2 + DOLFIN_EPS and on_boundary or \
				(x[0]-0.2)**2 + (x[1]-0.2)**2 < 0.05**2 + DOLFIN_EPS and \
				x[1] > 0.21 - DOLFIN_EPS and on_boundary or \
				(x[0]-0.2)**2 + (x[1]-0.2)**2 < 0.05**2 + DOLFIN_EPS and \
				x[1] < 0.19 + DOLFIN_EPS and on_boundary
...
# should FSI exclude nodes on left end? No, these nodes should have both BCs applied to them.
		class Fsi(SubDomain):
		    def inside(self,x,on_boundary):
		        return x[0] > 0.2 and x[0] < L - DOLFIN_EPS and \
		        x[1] > 0.19-DOLFIN_EPS and x[1] < 0.19 + DOLFIN_EPS and \
				on_boundary or \
				x[0] > 0.2 and x[0] < L - DOLFIN_EPS and \
		        x[1] > (0.21 - DOLFIN_EPS) and x[1] < (0.21 + DOLFIN_EPS) and \
				on_boundary or \
