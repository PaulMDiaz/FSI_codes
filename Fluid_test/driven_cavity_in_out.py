...
h = 0.5
W = 2.0
h_in = 0.1 # height of inlet and outlet
...
fsi = CompiledSubDomain('near(x[1], h)  && on_boundary', h = h)

left = CompiledSubDomain('near(x[0], 0.0) && x[1] <= H - h_in && on_boundary', H = H, h_in = h_in)
right = CompiledSubDomain('near(x[0],W) && x[1] <= H - h_in && on_boundary', W = W , H = H, h_in = h_in)

inlet = CompiledSubDomain('near(x[0], 0.0) && x[1] >= H - h_in && on_boundary', H = H, h_in = h_in)
outlet = CompiledSubDomain('near(x[0],W) && x[1] >= H - h_in && on_boundary', W = W, H = H, h_in = h_in)

# Define boundary conditions
noSlipLeft = DirichletBC(V, Constant((0.0, 0.0)), left)
noSlipRight = DirichletBC(V, Constant((0.0, 0.0)), right)

bcp_inlet = DirichletBC(Q, Constant(0), inlet)
bcp_outlet = DirichletBC(Q, Constant(0), outlet)

freestreamV = DirichletBC(V, U_top, top)

# Value of tauyy on inlet and outlet boundaries.
duydy = Constant(0.)
...
File('./Results_fluid_test/' + 'p1.pvd') << (p1)
