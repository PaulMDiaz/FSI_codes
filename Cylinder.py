...
class problem_specific:
	...
	def __init__(self):
		...
		self.T = 8.0	#  Set final time for iteration
		self.N = 128		# Number of discretizations (square mesh) (place cell edge on FSI)
		...
		self.struct_border = AutoSubDomain(lambda x: x[0] >= self.x_bar -self.s_b and x[0] <= 0.6 +self.s_b and x[1] >= 0.19 -self.s_b and x[1] <= 0.19+self.h+self.s_b)		#fsi = CompiledSubDomain('on_boundary && x[0] > x_bar + DOLFIN_EPS && x[0]< L - DOLFIN_EPS && x[1]>DOLFIN_EPS && x[1] < H-DOLFIN_EPS', L = self.L, H = self.H, x_bar = self.x_bar)

		# Refine mesh on structure, fluid and whole mesh... is whole mesh ever used?
		cell_markers = CellFunction("bool", self.mesh_s, False)
		self.struct_border.mark(cell_markers,True)
		...
		cell_markers = CellFunction("bool", self.mesh_f, False)
		self.struct_border.mark(cell_markers,True)
		self.mesh_f = refine(self.mesh_f, cell_markers)
	...
	def compute_forces(self,Mesh, p, ds):
		self.mesh = Mesh
	...
	def Define_Boundary_Conditions(self, S, F):
		...
		class cylinder_1(SubDomain):
		    def inside(self,x,on_boundary):
		        return x[0] > DOLFIN_EPS and x[0] < (0.25 + DOLFIN_EPS) and \
		        x[1] > DOLFIN_EPS and x[1] < (0.41 - DOLFIN_EPS)

		class fsi_1(SubDomain):
		    def inside(self,x,on_boundary):
		        return x[0] > 0.25-DOLFIN_EPS and x[0] < (2.5 - DOLFIN_EPS) and \
		        x[1] > DOLFIN_EPS and x[1] < (0.41 - DOLFIN_EPS)

		markers = FacetFunction("size_t", F.mesh, 1)
		cylinder_1().mark(markers,1);
		fsi_1().mark(markers,2);

		F.ds = ds(subdomain_data = markers)
