...
class StructureSolver:
	...
	def __init__(self, elementType, elementDegree, structureSolverMethod, structureBodyForce, p_s):
		...
		self.interfaceDispVectorSpace = VectorFunctionSpace(p_s.interfaceStructure, "CG", 1)

		self.u12, self.v12 = self.U.split(deepcopy = True)

		self.meshInterfaceDisplacement = interpolate(self.u12,self.interfaceDispVectorSpace)
		# self.meshInterfaceDisplacement = Function(self.interfaceDispVectorSpace)
		...
		# interfaceStructureDispCoords = self.interfaceDispVectorSpace.tabulate_dof_coordinates().reshape((self.interfaceDispVectorSpace.dim(),-1))
		self.interfaceStructureDispCoords = self.interfaceDispVectorSpace.tabulate_dof_coordinates().reshape((self.interfaceDispVectorSpace.dim(),-1))
...
# u11, v11 = structureSolver.U.split(deepcopy = True)
		...
		# interpolate mesh displacement to boundary for use as BC.
		self.meshInterfaceDisplacement = interpolate(self.u12,self.interfaceDispVectorSpace)
