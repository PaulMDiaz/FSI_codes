...
while p_s.t + p_s.dt<= p_s.T: # + DOLFIN_EPS:
	...
	for ii in range(3):
		...
		fluidSolver.solveFluidProblem(p_s, meshSolver.meshInterfaceVelocity, meshSolver.meshDisplacement)
	...
	# move fluid mesh and fluid FSI boundary mesh by respective mesh velocities.
	for i_coords in range(len(fluidSolver.mesh.coordinates())):
		fluidSolver.mesh.coordinates()[i_coords] += p_s.dt*meshSolver.meshVelocity(meshSolver.mesh.coordinates()[i_coords])

	for i_coords in range(len(p_s.interfaceFluid.coordinates())):
		p_s.interfaceFluid.coordinates()[i_coords] += p_s.dt*meshSolver.meshInterfaceVelocity(p_s.interfaceMesh.coordinates()[i_coords])
	...
	meshSolver.meshDisplacement0.vector()[:] = meshSolver.meshDisplacement1.vector().get_local()
	...
	for i_disp in range(u_s_FSI.size/2):
		# x
		disp_m_FSI[2*i_disp] = meshSolver.meshDisplacement(dispCoords[2*i_disp])[0]
		# y
		disp_m_FSI[2*i_disp+1] = meshSolver.meshDisplacement(dispCoords[2*i_disp])[1]
	...
	if count%500 == 0:
		...
		nodal_values_disp = meshSolver.meshDisplacement.vector().get_local()
		np.savetxt(pwd_restart+'nodal_m_disp', nodal_values_disp)
		nodal_values_disp = meshSolver.meshDisplacement1.vector().get_local()
		np.savetxt(pwd_restart+'nodal_m_disp1', nodal_values_disp)
		nodal_values_disp = meshSolver.meshDisplacement0.vector().get_local()
