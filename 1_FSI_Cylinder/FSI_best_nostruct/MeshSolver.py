...
class MeshSolver(object):
	...
	def __init__(self, meshElementType, meshElementDegree, p_s):
		...
		self.meshDisplacement = Function(self.vectorSpace)
		...
		# I think k gets overwritten at time step 1.
		self.k = Constant(0)
	...
	def meshProblemSolver(self, structureSolver, fluidSolver, p_s):
		...
		# using mesh displacement at time t
		self.meshDisplacement = 0.5*(self.u1+self.u0)
		self.meshInterfaceDisplacement = interpolate(self.u1,self.interfaceMeshVectorSpace)
		# or using mesh displacement midpoint

		# self.meshDisplacement.vector()[:] = 0.5*(self.u1.vector().get_local()+self.u0.vector().get_local())
		# self.meshInterfaceDisplacement = interpolate(self.meshDisplacement,self.interfaceMeshVectorSpace)
