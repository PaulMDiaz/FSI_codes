...
#i_f_V_fsi_t = np.where((dofs_f_V[:,1] == 0.2 + p_s.h/2.) & (dofs_f_V[:,0] >= p_s.x_bar) & (dofs_f_V[:,0] <= 0.6))[0]
#i_s_V_fsi_t = np.where((dofs_s_V[:,1] == 0.2 + p_s.h/2.) & (dofs_s_V[:,0] >= p_s.x_bar) & (dofs_s_V[:,0] <= 0.6))[0]
...
# order
#i_s_V_fsi1 = i_s_V_fsi1[dofs_s_V[i_s_V_fsi1][:,0].argsort()]
#i_f_V_fsi1 = i_f_V_fsi1[dofs_f_V[i_f_V_fsi1][:,0].argsort()]
...
for i_s in range(dofs_s_V[i_s_V_fsi].shape[0]/2):
	for i_f in range(dofs_f_V[i_f_V_fsi].shape[0]/2):
		...
		#if dofs_s_V[i_s_V_fsi[2*i_s]][0] == dofs_f_V[i_f_V_fsi[2*i_f]][0]:
################ Iteration section ##############################
...
count = 0
#F.sigma_FSI = Function(F.T_space)
...
#dofs_u_disp = dofs_s_S[i_s_S_fsi]
# matrix to store displacement in. One for a given time step, one to record displacment of previous time step.
disp_m_FSI_1 = 0*dofs_u_disp
disp_m_FSI_2 = 0*dofs_u_disp
...
while p_s.t <= p_s.T: # + DOLFIN_EPS:
	...
	for ii in range(3): #change to 3 to run properly.
		...
		# fluid velocity on FSI
		# fluid velocity, u, v, u, v etc
		u_f_FSI = F.u1.vector()[i_f_V_fsi_com]

		# Structure displacement and velocity on FSI
		u1, v1 = S.U.split(deepcopy = True)
		u_s_FSI = u1.vector()[i_s_V_fsi]
		v_s_FSI = v1.vector()[i_s_V_fsi]

		# mesh velocity on FSI
		u_m_FSI = F.u_mesh.vector()[i_f_V_fsi_com]

		#mesh displacement at same nodes as structure displacement
		#dofs_u_disp = dofs_s_V[i_s_V_fsi]
		# matrix to store displacement in
		#disp_m_FSI = 0*dofs_u_disp

		# step through each coordinate. (steps of two because vector)
		# for each coordinate add the displacement of this step to the displacement of previous step.
		#for i_disp in range(len(dofs_u_disp)/2):
			#disp_m_FSI_1[i_disp,:] = disp_m_FSI_2[i_disp,:] + p_s.dt*F.u_mesh(dofs_u_disp[i_disp])


		#F.u_mesh.vector()[i_f_V_fsi_com]
		#for x in F.mesh.coordinates(): x[:] += p_s.dt*F.u_mesh(x)[:]

		# dofs_f_V[i_f_V_fsi_com]
		# np.linalg.norm(dofs_s_V[i_s_V_fsi] - dofs_f_V[i_f_V_fsi_com])
		#alpha_1 = max([abs(max(u_m_FSI)), max(abs(u_f_FSI))])
		#if alpha_1 == 0:
			#alpha_1 = 1
		#alpha_2 = max([abs(max(u_m_FSI)), max(abs(v_s_FSI))])
		#if alpha_2== 0:
		#	alpha_2 = 1
		#alpha_3 = max([abs(max(u_f_FSI)), max(abs(v_s_FSI))])
		#if alpha_3 == 0:
		#	alpha_3 = 1
		#alpha_4 = max([abs(max(u_s_FSI)), max(abs(disp_m_FSI_1.flatten()))])
		#if alpha_4 == 0:
		#	alpha_4 = 1
		...
		#print "2 norm mesh and structure deflections :", np.linalg.norm(u_s_FSI/alpha_4 - disp_m_FSI_1.flatten()/alpha_4)
		...
		#print "2 norm mesh and structure deflections :", np.linalg.norm(u_s_FSI - disp_m_FSI_1.flatten())
	...
	#drag, lift = p_s.compute_forces(p_s.mesh_f,p_s.nu_f, F.u1, F.p1, F.ds)
	...
	print "ux, uy, drag and lift on cylinder and bar:", [u1(0.6,0.2)[0], u1(0.6,0.2)[1],drag, lift]
	...
	#u_fsi_t = F.u1.vector()[i_f_V_fsi_t]

	# things go bad when u_mesh is used as fluid BC...

	#u_inlet = F.u1.vector()[i_f_V_in]
	#print u_inlet
	#for x in F.mesh.coordinates(): x[:] += p_s.dt*F.u_mesh(x)[:]
	#p_s.Save_Results(S, F)
