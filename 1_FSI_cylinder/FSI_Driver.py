...
# top
i_s_S_T1 = np.where((dofs_s_S[:,1] == 0.2 + p_s.h/2.) & (dofs_s_S[:,0] >= p_s.x_bar) & (dofs_s_S[:,0] <= 0.6))[0]
i_f_S_T1 = np.where((dofs_f_S[:,1] == 0.2 + p_s.h/2.) & (dofs_f_S[:,0] >= p_s.x_bar) & (dofs_f_S[:,0] <= 0.6))[0]
# bottom
i_s_S_T2 = np.where((dofs_s_S[:,1] == 0.2 - p_s.h/2.) & (dofs_s_S[:,0] >= p_s.x_bar) & (dofs_s_S[:,0] <= 0.6))[0]
i_f_S_T2 = np.where((dofs_f_S[:,1] == 0.2 - p_s.h/2.) & (dofs_f_S[:,0] >= p_s.x_bar) & (dofs_f_S[:,0] <= 0.6))[0]

i_s_T_T2 = np.where((dofs_s_T[:,1] == 0.2 - p_s.h/2.) & (dofs_s_T[:,0] >= p_s.x_bar) & (dofs_s_T[:,0] <= 0.6))[0]
i_f_T_T2 = np.where((dofs_f_T[:,1] == 0.2 - p_s.h/2.) & (dofs_f_T[:,0] >= p_s.x_bar) & (dofs_f_T[:,0] <= 0.6))[0]
...
while p_s.t < p_s.T: # + DOLFIN_EPS:
	...
	for ii in range(3): #change to 3 to run properly.
		...
		print 'Time loop iteration number = ', count
	...
	F.p_n.assign(F.p_)		# set current time to previous for pressure.
	#S.d00_s.assign(S.d0)
...
#np.savetxt('nodal_u_64', nodal_values_u)
#nodal_values_p = F.p_.vector().array()
#np.savetxt('nodal_p_64', nodal_values_p)
#nodal_values_m = F.u_mesh.vector().array()
#np.savetxt('nodal_m_64', nodal_values_m)
