...
domain = f_domain+s_domain
domain.set_subdomain(1,s_domain)

mesh_all = generate_mesh(domain, N)

mesh = SubMesh(mesh_all, 0)
struct_border = AutoSubDomain(lambda x: x[0] >= x_bar - DOLFIN_EPS and x[0] <= 0.6 + DOLFIN_EPS and x[1] >= 0.19 - DOLFIN_EPS and x[1] <= 0.19+h+DOLFIN_EPS)
...
for i_mark in range(1):
    cell_markers_2 = FacetFunction("bool", mesh, False)
    struct_border.mark(cell_markers_2,True)
    mesh = refine(mesh, cell_markers_2)


for i_mark in range(0):
    cell_markers_2 = CellFunction("bool", mesh, False)
    refine_box.mark(cell_markers_2,True)
    mesh = refine(mesh, cell_markers_2)
...
scipy.io.savemat('results_cylinder.mat', mdict={'results':results})
