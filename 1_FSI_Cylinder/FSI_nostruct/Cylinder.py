...
class ProblemSpecific:
	...
	def __init__(self):
		...
		self.boundariesStructure = MeshFunction("size_t", self.meshStructure, self.meshStructure.topology().dim() - 1)
		self.boundariesStructure.set_all(0)
		self.fSIInterface.mark(self.boundariesStructure, 1)

		# restrict mesh to exterior boundaries only:
		boundaryStructure = BoundaryMesh(self.meshStructure, 'exterior')
		self.interfaceStructure = SubMesh(boundaryStructure, self.fSIInterface)
 ...
 # boundaries already defined in geometry file...
	def defineBoundaryConditions(self, structureSolver, fluidSolver, meshSolver, p_s):
		...
		# bcMeshFSI = DirichletBC(meshSolver.vectorSpace, structureSolver.meshInterfaceDisplacement, p_s.facetsRef, 20)

		# define expression for FSI boundary
		class MeshInterfaceDisplacementExpression(Expression):
		    def eval(self, values, x):
		        try:
		            values[:] = structureSolver.meshInterfaceDisplacement(x)
		        except:
		            values[:] = 0
		    def value_shape(self):
		        return (2,)

		meshInterfaceDisplacementExpression = MeshInterfaceDisplacementExpression(degree=2)

		bcMeshFSI = DirichletBC(meshSolver.vectorSpace, meshInterfaceDisplacementExpression, p_s.facetsRef, 20)
		# bcMeshFSI = DirichletBC(meshSolver.vectorSpace, Constant((0,0)), p_s.facetsRef, 20)
	...
	def defineInterfaceDofs(self, structureSolver, fluidSolver, meshSolver, p_s):
		...
		# repalce dofs_s_V with dofs_disp to test mesh deformation
		self.dofs_s_V = structureSolver.interfaceDispVectorSpace.tabulate_dof_coordinates().reshape((structureSolver.interfaceDispVectorSpace.dim(),-1))
		# indices should now be every dof
		self.i_s_V_fsi = range(self.dofs_s_V.shape[0])
	...
	def saveResults(self, structureSolver, fluidSolver, meshSolver):
		...
		# meshSolver.v_res.assign(meshSolver.meshVelocity)
		# self.file_v_m << (meshSolver.v_res, self.t)

		meshSolver.u_res.assign(meshSolver.u1)
		...
		self.file_u_current << (fluidSolver.u_current, self.t)
