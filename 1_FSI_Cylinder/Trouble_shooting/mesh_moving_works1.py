...
# assigning values. May have to
# Setting up SubDomain - really should not
# Creating bc - should not, but requires subdomain
...
# bcMove = DirichletBC(thirdVectorSpace,thirdInterfaceValues, thirdInterface)
bcMove = DirichletBC(thirdVectorSpace,thirdInterfaceValues, boundariesMesh3, 1)

bcMove.apply(thirdSolution.vector())

# dofsThird = thirdVectorSpace.tabulate_dof_coordinates().reshape((thirdVectorSpace.dim(),-1))
# indicesThird = np.where((dofsThird[:,1] <= 2.0 + DOLFIN_EPS)&(dofsThird[:,1] >= 2.0 - DOLFIN_EPS))[0]
...
# indicesSecond
# dofsThird[indicesThird]
# dofsThird[indicesRef]
